...
const getParagraphs = async (): Promise<Array<CodeofconductAd>> => {
  const { data, error } = await supabase.from('codeofconduct_ads').select('*').order('id', { ascending: true });
  if (error) {
    console.log(error);
    return [{
      id: null,
      ...
      <div>
        {
          codeofconductAds.map((codeofconductAd, idx) => {
            ...
            if ((idx > 0 && codeofconductAds[idx - 1].title?.trim() == codeofconductAd.title?.trim())
              || (cnt == 0 && idx < codeofconductAds.length - 1 && codeofconductAds[idx + 1].title == codeofconductAd.title)) {
              if (cnt != 0) {
                isFirst = false;
              }
              ...
              cnt = 0;
              if (idx < codeofconductAds.length - 1 && codeofconductAds[idx + 1].title?.trim() == codeofconductAd.title?.trim()) {
                isFirst = true;
                prefix = String.fromCharCode(cnt + 97) + '. ';
                cnt++;
              }
        ...
        <div className='mt-[50px] text-justify'>
          <span className='text-adDescBigSize md:text-adDescSmallSize sm:text-adDescSmallSize text-bannerTextColor font-arial font-light'>
            ...
            in all our interactions and work.
          </span>
        </div>
      </div>
